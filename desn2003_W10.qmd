...
# Where This Fits Now {background-color="#E9C46A"}

## This Is Primarily Doc 1 Support

::: {.callout-note icon=false}
Because the calendar is compressed:

- **Mar 31**: Professional Discovery presentations begin
- **Apr 7**: no class
- **Apr 14**: **Doc 0.2** + **Professional Discovery Documentation**
- **Apr 21**: **Doc 1 due** + final presentations start
:::

That means this visualization deck is less about "one more theory lecture" and more about helping you present findings clearly in **Doc 1** and in your final presentation.

If your data is not collected yet, use this session selectively: decide **what figures you will need later** based on the method you already chose in Week 8.
...
- **Programming Libraries:** Python’s **Matplotlib/Seaborn** are strong for reproducible static charts, **Plotly** is a practical route to interactive notebook visuals, and **D3.js** is the web-native option when you need fully custom interactive storytelling.
- **Which are easiest?** For absolute beginners, spreadsheets and GUI tools are most accessible. For this class, Python is the practical middle ground: easier than D3.js, but more flexible than spreadsheets.

## From Static Charts to Interactive Stories

- **Plotly:** Best next step if you already have data in Python and want hover labels, zooming, Sankey diagrams, treemaps, and maps without leaving the notebook.
- **D3.js:** Most flexible if you want custom interaction, animation, or a scrollytelling website. It is powerful, but it is also the steepest learning curve.
- **Tableau / Power BI / Flourish:** Fastest route to presentation-ready dashboards and embeddable interactive visuals when coding is not the main goal.
- **Rule of thumb for DESN2003:** Start with clear static charts. Move to interactive tools only when the interaction itself helps explain the insight.
...
![](data/week10_figures/bar_histogram_examples.png){fig-alt="Side-by-side bar chart and histogram using the Week 10 teaching dataset." width=90%}

*Left: a bar chart comparing recommendation outcomes by group. Right: a histogram showing the distribution of scores. Same dataset, different question.*
...
![](data/week10_figures/box_plot_scores.png){fig-alt="Example box plot comparing score distributions across GenAI and Traditional groups." width=72%}

*Example of a box plot comparing test score distributions for two groups (GenAI vs Traditional). The median score for the GenAI group is higher, and the distribution is slightly tighter than the Traditional group.*
...
![](data/week10_figures/heatmap_examples.png){fig-alt="Side-by-side correlation heatmap and group-versus-recommend matrix heatmap." width=90%}

*Left: a correlation heatmap. Right: a confusion-matrix-style table showing how recommendation outcomes differ across groups.*
...
![](data/week10_figures/treemap_region_group.png){fig-alt="Treemap showing student counts grouped by region and learning method." width=84%}

*This treemap uses the same teaching dataset, showing how a hierarchical view can reveal both region size and group composition at once.*

## Sankey Diagrams

- **What is a Sankey diagram?** A Sankey diagram visualizes **flows between stages**. The width of each band represents how much of something moves from one state to another.
- **Use cases:** Great for journeys, funnels, decision paths, or any process with transitions. In design research, this could mean participant flow, interview coding pathways, or movement from one category to another.
- **In our example:** We can show a flow from **Group → Satisfaction Band → Recommendation**. That is much harder to read from a raw table, but easy to scan in a Sankey.
- **Tool note:** Sankey diagrams are often built in **Plotly**, **Flourish**, or **D3.js** when you want a more interactive version.

![](data/week10_figures/sankey_group_satisfaction_recommend.png){fig-alt="Sankey diagram showing flow from group to satisfaction band to recommendation." width=84%}

*This is the kind of chart you reach for when the story is about movement or transition, not just counts.*
...
![](data/week10_figures/choropleth_representative_countries.png){fig-alt="Choropleth demonstration using representative country codes derived from the Week 10 dataset's regions." width=86%}

*This demo maps region-level data to representative country codes so you can see the mechanics of a choropleth. For real work, you would want actual country, district, or postcode data.*
...
![](data/week10_figures/wordcloud_transcripts.png){fig-alt="Word cloud built from the Week 10 interview transcript sample." width=76%}

*Word cloud of common terms from student transcripts. Frequently mentioned words appear larger, indicating prevalent themes.*
...
*Now that we've covered the theory, let's put it into practice with some hands-on exercises. Open the [Week 10 notebook](DataVisualization_Notebook.ipynb) or launch it in [Google Colab](https://colab.research.google.com/github/clementinec/desn2003/blob/main/DataVisualization_Notebook.ipynb), then work through the examples step-by-step.* 🚀
